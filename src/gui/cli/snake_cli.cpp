...
void View::renderField(WINDOW *field, GameInfo_t game) {
  werase(field);                   // Очистка окна
  box(field, 0, 0);                // Отрисовка рамки окна
  int **coordinates = game.field;  // Получение текущего состояния игрового поля
  ...
  if (coordinates) {
    for (int i = 0; i < 20; i++) {
      for (int j = 0; j < 10; j++) {
        ...
        } else if (coordinates[i][j] == 4) {  // Если это голова змейки
  ...
  wrefresh(field);  // Обновление окна для отображения изменений
}
